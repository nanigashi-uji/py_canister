...
When the script `py_canister` is invoked with `--manage` as the **first** option and `init` as the sub-command, this script works as `init-mode`. 
...
```
% ./py_canister --manage -p /somewhere/pyworktool -3 init pywork
% (cd /somewhere/ ; find pyworktool -ls )
 drwxr-xr-x   ...... pyworktool
 drwxr-xr-x   ...... pyworktool/bin
 -rwxr-xr-x   ...... pyworktool/bin/py_canister
 lrwxr-xr-x   ...... pyworktool/bin/mng_pyenv -> py_canister
 lrwxr-xr-x   ...... pyworktool/bin/mng_pyenv2 -> py_canister
 lrwxr-xr-x   ...... pyworktool/bin/mng_pyenv3 -> py_canister
 lrwxr-xr-x   ...... pyworktool/bin/pywork -> py_canister
 lrwxr-xr-x   ...... pyworktool/bin/pywork3 -> py_canister
...
`mng_pyenv` and `mng_pyenv[23]` made by `py_canister` with `init-mode` is symbolic link to `bin/py_canister`. Invoking `mng_pyenv` is equivalent to invoke `py_canister --manage`. (Invoking `mng_pyenv2` and `mng_pyenv3` are equivalent to invoke `py_canister --manage -2` and `py_canister --manage -3`, respectively.) In this mode, other sub-commands are also available. If it is invoked with sub-command `install` and python module name(s) in following arguments, the specified python modules are installed under the local directory structure.
...
If the symbolic link to this script(`py_canister`) with the name other than `mng_pyenv{,2,3}` is invoked, the python script, `lib/python/{symbolic_link_name}.py`, is invoked with setting environmental variable `PYTHONPATH` to use python modules installed locally. If `py_canister` is invoked with first argument other than `--manage`, the python script, `lib/python/{first_arguments}.py` or `{first_arguments}` is invoked with setting `PYTHONPATH`.
...
% /somewhere/pyworktool/bin/py_canister pywork3 Japan
Hello, Japan!
Python : 3.8.9 (/Applications/Xcode.app/Contents/Developer/usr/bin/python3)
....

% /somewhere/pyworktool/bin/py_canister  /somewhere/pyworktool/lib/python/pywork3.py  Japan
Hello, Japan!
Python : 3.8.9 (/Applications/Xcode.app/Contents/Developer/usr/bin/python3)
....
```
...
```
% ./py_canister --manage -p /somewhere/pyworktool -g -r -t 'Worktool by Python' -3 -m pytz -m tzlocal init prepwork mainwork
Collecting pytz
  Using cached pytz-2022.7.1-py2.py3-none-any.whl (499 kB)
Collecting tzlocal
  Using cached tzlocal-4.2-py3-none-any.whl (19 kB)
...
...... pyworktool/bin
...... pyworktool/bin/py_canister
...... pyworktool/bin/mng_pyenv -> py_canister
...... pyworktool/bin/mng_pyenv2 -> py_canister
...... pyworktool/bin/mng_pyenv3 -> py_canister
...... pyworktool/bin/prepwork -> py_canister
...... pyworktool/bin/prepwork3 -> py_canister
...... pyworktool/bin/mainwork -> py_canister
...... pyworktool/bin/mainwork3 -> py_canister
...... pyworktool/lib
...
- Contents:
  ...
  2.  bin/mng_pyenv:                     Symblic link to 'py_canister' for installing Python modules by pip locally.
  2a. bin/mng_pyenv2:                    Same as above using pip2 as default
  2b. bin/mng_pyenv3:                    Same as above using pip3 as default

  3.  bin/py_canister:                      Wrapper bash script to invoke Python script. (Entity)
  ...
  6.  bin/prepwork:                      Symbolic link to 'py_canister' to invoke prepwork.py.
  ...
  8.  bin/mainwork:                      Symbolic link to 'py_canister' to invoke mainwork.py.
...
`add` sub-command in `manage-mode` can be used to add the new python script template, the symbolic link to `bin/py_canister` with corresponding name, and to update `README.md` when `-r` option is given.

```
% /somewhere/pyworktool/bin/mng_pyenv -r add postwork
% ( cd /somewhere ; find pyworktool -ls ) ...... 
...... pyworktool/bin/postwork3 -> py_canister
...... pyworktool/bin/postwork -> py_canister
...... 
...
......
  ...
  12.  bin/postwork:                      Symbolic link to 'py_canister' to invoke postwork.py.
...
```
UserA@HostA % cp -ai 'py_canister' '/somewhere/tool-dist'
...
```
% ./py_canister --manage -h
[Usage]        % mng_pyenv{,2,3}   [options] sub-command [arguments]
               % py_canister --manage [options] sub-command [arguments]
[sub-commands] 
               % py_canister --manage [options] init [scriptnames] ... setup directory tree,
                                                                        and prepare templates if arguments are given.
               % py_canister --manage [options] add script_names   ... prepare python script templates.
               % py_canister --manage install  module_names        ... Install python module with pip locally.
               % py_canister --manage download module_names        ... Download python module with pip locally.
               % py_canister --manage clean                        ... Delete local python module for corresponding python version.
               % py_canister --manage distclean/cleanall/allclean  ... Delete local python module for all python version.
               % py_canister --manage info                         ... Show information.
[Options]
               -h     : Show this message
...
[Options for sub-command: setup/init ]
               -p     : prefix of the directory tree. (Default: Grandparent directory if the name of parent directory of 
                        py_canister is bin, otherwise current working directory.)
